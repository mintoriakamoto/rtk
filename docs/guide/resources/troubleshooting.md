...
**Cause:** You installed **Rust Type Kit** (`reachingforthejack/rtk`) instead of **Rust Token Killer** (`mintoriakamoto/rtk`). They share the same binary name.
...
```bash
cargo install --git https://github.com/mintoriakamoto/rtk --branch master
...
Open an issue: https://github.com/mintoriakamoto/rtk/issues
