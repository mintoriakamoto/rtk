...
pub fn run_subcommand(cmd: &PortalSubcommand, verbose: u8) -> Result<i32> {
    match cmd {
        ...
        PortalSubcommand::Logout => {
            let path = credentials_path()?;
            // Removing this file is the entire point of `logout`: the device
            // token must not outlive it. The path is the config dir plus a
            // fixed filename — never user input, so there is no traversal or
            // arbitrary-deletion surface.
            // nosemgrep: filesystem-deletion -- deletes only rtk's own credentials file
